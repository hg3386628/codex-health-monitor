...
> 当前发布产物面向 `linux/arm64`，需要支持标准动态库插件的 CPA 版本。
...
1. 从项目的 [Releases](https://github.com/hg3386628/codex-health-monitor/releases) 页面下载 `codex-health-monitor.so`。

2. 将动态库复制到 CPA 的插件目录。以下示例假设 CPA 安装在 `/opt/cli-proxy-api`，且 `plugins.dir` 使用默认值 `plugins`：

```bash
sudo install -D -m 0755 codex-health-monitor.so \
  /opt/cli-proxy-api/plugins/linux/arm64/codex-health-monitor.so
```

CPA 会依次查找 `<plugins.dir>/linux/arm64/` 和 `<plugins.dir>/`。如果你的 `plugins.dir` 是绝对路径，请相应替换上面的目标目录。
...
| `interval_min` | `30` | 检测间隔，范围为 5 到 10080 分钟 |
...
仓库提供的脚本会在 `golang:1.24-bookworm` 容器中运行测试并交叉构建 `linux/arm64` 动态库：
...
```text
dist/codex-health-monitor.so
```

可通过环境变量覆盖版本和 Go 镜像：

```bash
VERSION=0.1.2 GO_IMAGE=golang:1.24-bookworm ./build.sh
```
...
- 插件没有出现在列表：确认系统架构是 `linux/arm64`、文件名为 `codex-health-monitor.so`，并检查 `plugins.dir` 与实际挂载路径。
